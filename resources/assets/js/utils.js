...
var utils= {
    ...
    'digitsBetween': function (min, max, val) {
        utils.tipInfo("按要求填写信息")
        if ( utils.isNumber(val) ) {
            if ( val >= min && val <= max ) {
                utils.tipValid();
                return true;
            } else {
                utils.tipWarn("填写的数字范围有误!");
                return false;
            }
        }else {
            utils.tipWarn("智能填写数字!");
            return false;
        }
    },
    ...
    'isMobile': function (val) {
        utils.tipInfo("填写正确的手机号");
        reg= /^(0|86|17951)?(13[0-9]|14[0-9]|17[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/;
        if(!reg.test(val)) {
            utils.tipWarn("手机号码格式错误!");
            return false;
        }
        utils.tipValid();
        return true;
    ...
    'isEmail': function (val) {
        utils.tipInfo("填写正确的邮箱号");
        reg= /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\.[a-zA-Z0-9_-]{2,3}){1,2})$/;
        if(!reg.test(val)) {
            utils.tipWarn("邮箱格式错误!");
            return false;
        }
        utils.tipValid();
        return true;
    ...
    'isNumber': function (val) {
        utils.tipInfo("填写正确的数值");
        reg= /^\d+$/;
        if(!reg.test(val)) {
            utils.tipWarn("只能填写数字!");
            return false;
        }
        utils.tipValid();
        return true;
    ...
    // 限制字符长度
    'length': function (size, val) {
        utils.tipInfo("按要求填写内容");
        if ( String(val).length >= 0 && String(val).length <= size ) {
            utils.tipValid();
            return true;
        } else {
            utils.tipWarn("字符长度超出范围!");
            return false;
        }
    },
    // 字符串为必填字段 required
    ...
    // 表单验证模块
    'validField': function (id) {
        // 获取vaildrule内容 每个input都bind执行一次validField  参数是$this 
        if ( $(this).attr('type') == 'text' ) {
            var rule = id.attr('validrule');
            var rules = rule.split('|', rule); 
            rules.forEach(function (field){
                utils[field]();
            })
    ...
    'hideLabel': function (){
        if ( $(this).attr('type') == 'text' ) {
            ...
            $(this).after('<input type="hidden" name="preview_'+new_id+'" value="" />');
        }
        ...
        if ( $(this).attr('type') == 'file'  ) {
            ...
            if (picurl) {
                $('#preview_'+new_id+'').attr('src', picurl);
                continue;
            }
            var fileObj = $(this).prop('files');
            if (fileObj) {
                if ($(this).prop('files')) {
                    var f = $(this).prop('files')[0];
                    if(f){
                        $('#preview_'+new_id+'').attr('src', URL.createObjectURL(f));
                    }
                }
            }
            continue;
    ...
    // 添加提示信息 tips
    'tipInfo': function (tipInfo) {
        $(this).siblings('.tips').html('<span class="cue">'+tipInfo+'</span>');
    },
    'tipWarn': function (tipWarn) {
        $(this).siblings('.tips').html('<span class="unuse">'+tipWarn+'</span>');
    },
    'tipValid': function () {
    ...
    'cityChange': function (){
        // 获取最外层ID  省+市+区
        var classA = $(this).find('select').eq(0);
        var classB = $(this).find('select').eq(1);
        var classC = $(this).find('select').eq(2);
        // 循环添加内容
        var A = utils.city;
